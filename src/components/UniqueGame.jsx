...
const UniqueGame = ({ onBack }) => {
  ...
  const generatePuzzle = (difficulty) => {
    ...
    let uniqueItemType = 'both' // Default to unique shape + color
    if (difficulty < 3) {
      const randomType = Math.random()
      if (randomType < 0.4) {
        uniqueItemType = 'shape' // Unique shape, color can be duplicated
      } else if (randomType < 0.8) {
        uniqueItemType = 'color' // Unique color, shape can be duplicated
      } else {
        uniqueItemType = 'both' // Unique shape + color
      }
    }
    ...
    if (uniqueItemType === 'shape') {
      ...
      uniqueShape = shapes[Math.floor(Math.random() * shapes.length)]
      isUniqueColor = true
    } else { // uniqueItemType === 'both'
      // Ensure unique shape + color combination
      do {
        uniqueShape = shapes[Math.floor(Math.random() * shapes.length)]
        uniqueColor = colors[Math.floor(Math.random() * colors.length)]
      } while (combinations.some(c => c.shape === uniqueShape && c.color === uniqueColor))
      isUniqueShape = true
      isUniqueColor = true
    }
  ...
  const selectItem = (itemId) => {
    ...
    let isCorrect = false
    if (currentPuzzle.difficulty < 3) { // For easier levels, check for unique shape, color, or both
      if (selectedItem.isUniqueShape && selectedItem.isUniqueColor) {
        isCorrect = true // Both shape and color are unique
      } else if (selectedItem.isUniqueShape && !selectedItem.isUniqueColor) {
        isCorrect = true // Only shape is unique
      } else if (!selectedItem.isUniqueShape && selectedItem.isUniqueColor) {
        isCorrect = true // Only color is unique
      }
    } else { // For harder levels, it's always unique shape + color
      isCorrect = selectedItem.isUnique
    }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Find the unique shape that doesn't belong!
              </CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <div className="lg:col-span-2">
            {gameActive && currentPuzzle && (
              <Card className="h-full">
                <CardHeader>
                  <CardTitle className="text-center">Find the unique shape!</CardTitle>
                  <CardDescription className="text-center">
                    Level {level} - Click on the shape that's different from the others
                  </CardDescription>
                </CardHeader>
                <CardContent className="flex flex-col items-center justify-center space-y-6">
                  ...
                  <div className="text-sm text-gray-600 text-center max-w-md">
                    Find the shape with a unique combination of form and color! 
                    Look for the one shape that doesn't have any duplicates.
                  </div>
                </CardContent>
              </Card>
            )}
                ...
                <CardContent className="flex flex-col items-center justify-center min-h-96 space-y-6">
                  <div className="text-center space-y-4">
                    ...
                    <p className="text-lg text-gray-600 max-w-md">
                      Test your visual attention skills! Find the one shape that has a unique 
                      combination of form and color. While other shapes may appear multiple times, 
                      only one will be completely unique.
                    </p>
                    <div className="bg-blue-50 p-4 rounded-lg">
                      <p className="text-sm text-blue-800">
                        <strong>How to play:</strong> Look for the shape that appears only once 
                        with its specific combination of form and color. For example, if there are 
                        3 red circles and 2 blue squares, but only 1 green triangle - the green triangle is unique!
                      </p>
                      <p className="text-sm text-blue-800 mt-2">
                        In harder levels, some shapes might spin, and the unique item could be unique 
                        in its shape, color, or both!
                      </p>
                    </div>
                  </div>
